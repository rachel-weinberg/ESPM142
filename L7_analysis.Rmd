...
#Box 2. View the summary statistics of the time it took for ants of each type to move over the bridge 
Note that there is a separate value for each colony!
```{r}

dt_times<-dt[,c("colony", "time_first_ant", "time_first_queen", "time_first_brood")]

dt_time_summary<- dt_times %>% group_by(colony) %>% summarise(firstworker_mean=mean(time_first_ant), firstworker_sd=sd(time_first_ant), firstqueen_mean=mean(time_first_queen), firstqueen_sd=sd(time_first_queen), firstbrood_mean=mean(time_first_brood), firstbrood_sd=sd(time_first_brood)) %>% pivot_longer(-colony, names_to=c("type", ".value"), names_sep="_")

dt_time_summary
```

#Box 3. Plot time to first ant of each type crossing 
This will plot the mean and standard deviation of the time it took from bridge placement to the first crossing of each ant type: workers, queens, and brood.
...
As in the plot above, this code will generate a plot showing the mean and standard deviation of the crossing rate for each colony at each of the different flow rates. Give each plot a descriptive title by replacing "My plot title" in the first and second line. The first plot (enter title in line 62) will display crossing rates separately for each colony and the second plot (enter title in line 63) will display the mean crossing rate for both colonies at each flow rate.
...
plot_rate_bar<-ggplot(data=dt_flow_summary, aes(x=flowrate, y=mean, fill=colony))+ 
...
The first result (anova_fw) is the ANOVA for time to first worker, the second (anova_fq) is for the time to first queen, and the third (anova_fb) is for the time to first brood
```{r}
