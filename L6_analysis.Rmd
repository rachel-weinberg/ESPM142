...
PI2<-preference_index(item1=dt$total_time_2[dt$Assay=="host.history"], item2 = dt$total_time_1[dt$Assay=="host.history"])

PI3<-preference_index(item1=dt$total_time_2[dt$Assay=="visual.vs.chemical"], item2 = dt$total_time_1[dt$Assay=="visual.vs.chemical"])

PI4<-preference_index(item1=dt$total_time_2[dt$Assay=="visual.with.chemical"], item2 = dt$total_time_1[dt$Assay=="visual.with.chemical"])

PI5<-preference_index(item1=dt$total_time_2[dt$Assay=="chemical.parasitized"], item2 = dt$total_time_1[dt$Assay=="chemical.parasitized"])

PI6<-preference_index(item1=dt$total_time_2[dt$Assay=="chemical.conspecific.larva"], item2 = dt$total_time_1[dt$Assay=="chemical.conspecific.larva"])
...
PI_plot<-ggplot(data=dt %>% pivot_longer(cols=c(PI1, PI2), names_to="item", values_to = "PI"), aes(x=Assay, y=PI, fill=item)) +geom_boxplot()+theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1))+ggtitle(PI_plot_title)
PI_plot
```
...
```{r}
sumstats<-dt %>% group_by(Assay) %>% summarise(mean_SOF_item_1=mean(SOF_1), sd_SOF_item_1=sd(SOF_1), variance_SOF_item_1<-mean_CAP_item_1=mean(CAP_1), sd_CAP_item_1=sd(CAP_1), mean_SOF_item_2<-mean(SOF_2), sd_SOF_item_2=sd(SOF_2), mean_CAP_item_2=mean(CAP_2), sd_CAP_item_2=sd(CAP_2))
sumstats
```
