...
```{r}
myinsects<-c("insect1", "insect2", "insect3", "insect4", "fly", "insect6") #edit this line

colnames(dat.obs)<-c("flower", "behavior", myinsects)
```
...
1. replace the link next to "target.sheet" with the link for the data sheet with the class data for your target insect
...
```{r}
target.sheet<-"https://docs.google.com/spreadsheets/d/195c-B16c0_oPB6jzFdqbyFIQJyvIFygHkB5dFZzqYeM/edit?usp=sharing"

broad.sheet<-"https://docs.google.com/spreadsheets/d/1WY1oOaE7N0a8MSWCiE4RnP-GO1z2a709OZIma5PYL8c/edit?usp=sharing"
...
```{r}
colnames(datb)<-c("site", "behavior", myinsects)
```

#Box 3A. Plot behavioral observations
```{r}
datlb<-datb %>% pivot_longer(cols=myinsects, names_to ="insect")
...
The first several lines are just putting the data for your broad range flower and your observed flowers together in the same format. 
...
Remember, results are usually considered "significantly different" if the p-value is less than 0.05
```{r}
#formatting lines
...
This plot will allow you to visualize the types of insects that landed on your flower designed for your target insect.
```{r}
...
Replace "my_insect" below with the name of your target insect as it appears on both sheets. Make sure it stays in quotation marks!
